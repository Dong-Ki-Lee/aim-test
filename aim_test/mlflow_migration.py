...
def parse_mlflow_logs(tracking_uri, experiment_name, run):
    while True:
        try:
            parse_mlflow_logs_test(tracking_uri, experiment_name, run)
        except Exception as e:
            print(e)
            break
        break

def parse_mlflow_logs_test(tracking_uri, experiment_name, run):

    client = mlflow.tracking.client.MlflowClient(tracking_uri=tracking_uri)

    run_id = run.info.run_id
    aim_run = Run(
        repo='aim://43.200.116.230:53800',
